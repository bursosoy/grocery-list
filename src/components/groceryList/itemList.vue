  ...
  <div>
    <v-list flat class="pa-0">
      <v-list-item-group multiple>
        <div v-for="(item, index) in filteredGroceryItems" :key="index" @contextmenu="show(item, $event)">
          <v-list-item @change="toggleSelected(item)" :class="{selected: item.isSelected}" class="list-item">
            <template v-slot:default="{active}">
              <v-list-item-action class="ml-0 mr-2 draggable" v-if="!isAlphaSorted">
              ...
              <v-list-item-action class="mr-0" :class="!isAlphaSorted && 'list-check'">
    ...
    <blockquote class="empty pa-4" v-if="!filteredGroceryItems.length">Your list is empty!</blockquote>
...
export default {
  ...
  computed: {
    filteredGroceryItems() {
      return this.$store.getters.filteredGroceryItems
    },
    isAdding() {
  ...
  watch: {
    filteredGroceryItems() {
      !this.filteredGroceryItems.length &&
        (() => {
